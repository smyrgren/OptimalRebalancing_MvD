            ...
            switch obj.funcName
                case 'MV'
                    if ~getOptimal % Optimize with transaction costs
                        [dw, ~, obj.exitFlag] = fmincon(@(dw)cfMV_wTC(dw, obj.curWts',  obj.expRts, ...
                                            obj.tnsCst, obj.lambdaAbs, obj.covMtx), ...
                                    dw0, A, b, Aeq, beq, lb, ub, nonlincons, options);
                            
                        obj.newWts = obj.curWts + (dw(1:end/2) - dw(end/2+1:end))';
                        obj.funcValCur = cfMV(obj.curWts', obj.expRts, obj.lambdaAbs, ...
                                             obj.covMtx);
                        obj.funcValNew = cfMV(obj.newWts', obj.expRts, obj.lambdaAbs, ...
                                             obj.covMtx);
                        obj.paidTC = obj.tnsCst * abs(obj.curWts - obj.newWts)';
                    else % Optimize without transaction costs -> OPTIMAL ALLOCATION
                        [dw, ~, obj.exitFlag] = fmincon(@(dw)cfMV_wTC(dw, obj.curWts', obj.expRts, ...
                                            zeros(size(obj.tnsCst)), obj.lambdaAbs,  obj.covMtx), ...
                                    dw0, A, b, Aeq, beq, lb, ub, nonlincons, options);
                            
                        obj.optWts = obj.curWts + (dw(1:end/2) - dw(end/2+1:end))';
                        obj.funcValCur = cfMV(obj.curWts',  obj.expRts, obj.lambdaAbs, ...
                                             obj.covMtx);
                        obj.funcValOpt = cfMV(obj.optWts',  obj.expRts, obj.lambdaAbs, ...
                                             obj.covMtx);
                    end
                case 'MVT'
                    if ~getOptimal % Optimize with transaction costs
                        [dw, ~, obj.exitFlag] = fmincon(@(dw)cfMVT_wTC(dw, obj.curWts', obj.benWts', obj.expRts, ...
                                            obj.tnsCst, obj.lambdaAbs, obj.lambdaRel, obj.covMtx), ...
                                    dw0, A, b, Aeq, beq, lb, ub, nonlincons, options);
                            
                        obj.newWts = obj.curWts + (dw(1:end/2) - dw(end/2+1:end))';
                        obj.funcValCur = cfMVT(obj.curWts', obj.benWts', obj.expRts, obj.lambdaAbs, ...
                                            obj.lambdaRel, obj.covMtx);
                        obj.funcValNew = cfMVT(obj.newWts', obj.benWts', obj.expRts, obj.lambdaAbs, ...
                                            obj.lambdaRel, obj.covMtx);
                        obj.paidTC = obj.tnsCst * abs(obj.curWts - obj.newWts)';
                    else % Optimize without transaction costs
                        [dw, ~, obj.exitFlag] = fmincon(@(dw)cfMVT_wTC(dw, obj.curWts', obj.benWts', obj.expRts, ...
                                            zeros(size(obj.tnsCst)), obj.lambdaAbs, obj.lambdaRel, obj.covMtx), ...
                                    dw0, A, b, Aeq, beq, lb, ub, nonlincons, options);
                            
                        obj.optWts = obj.curWts + (dw(1:end/2) - dw(end/2+1:end))';
                        obj.funcValCur = cfMVT(obj.curWts', obj.benWts', obj.expRts, obj.lambdaAbs, ...
                                            obj.lambdaRel, obj.covMtx);
                        obj.funcValOpt = cfMVT(obj.optWts', obj.benWts', obj.expRts, obj.lambdaAbs, ...
                                            obj.lambdaRel, obj.covMtx);
                    end
                case 'QAprx' % We use MVT for simplicity MV = MVT with lambdaRel = 0
                    if ~getOptimal % Optimize with transaction costs and QAprx
                        [dw, ~, obj.exitFlag] = fmincon(@(dw)cfMVT_wTC_wQAprx(dw, obj.curWts', obj.benWts', ...
                                           obj.optWts', obj.expRts, obj.tnsCst, obj.lambdaAbs, ... 
                                            obj.lambdaRel, obj.covMtx, obj.QAprx), ...
                                    dw0, A, b, Aeq, beq, lb, ub, nonlincons, options);
                        ...
                        [dw, ~, obj.exitFlag] = fmincon(@(dw)cfMVT_wTC_wQAprx(dw, obj.curWts', obj.benWts', ...
                                            zeros(1,numAssets)', obj.expRts, zeros(size(obj.tnsCst)), obj.lambdaAbs,  ...
                                            obj.lambdaRel, obj.covMtx, zeros(size(obj.QAprx))), ...
                                    dw0, A, b, Aeq, beq, lb, ub, nonlincons, options);
...
% Actual costfunctions
% Mean-variance error
function f = cfMV(w, mu, lambda_abs,  cov_matrix)

    f = -mu*w + lambda_abs*w'*cov_matrix*w;
...
% Mean-variance with tranaction costs
function f = cfMV_wTC(dw, wc, mu, tnsCst, lambdaAbs,  covMtx)
...
function f = cfMVT_wTC(dw, wc, wb, mu, tnsCst, lambdaAbs,  lambdaRel, covMtx)
...
% Mean-variance-tracking with tranaction costs
function f = cfMVT_wTC_wQAprx(dw, wc, wb, wo, mu, tnsCst, lambdaAbs,  lambdaRel, covMtx, QAprx)
